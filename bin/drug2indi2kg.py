...
drugIndicationsWithNDAs = FAERSdir+'/results/drug-indi-nda.txt.gz'
...
seen = {}
with gzip.open(drugIndicationsWithNDAs, 'rt') as dindafile:
	for line in dindafile:
		n, nda, drugname, ingredient, indication = line.strip().split('\t')
		objects = interpretConditions('', indication)
		if not objects:
			if indication in seen: continue
			seen[indication] = 1
			doLog('cannot interpret condition', indication)
			#print('cannot interpret condition', indication, flush=True)
			continue
		try: curie = term2curie[indication]
			...
			continue
		if not curie in supportInDailyMed:
			if curie in seen: continue
			seen[curie] = 1
			#print('nothing in DailyMed for', curie, flush=True)
		...
		if not re.search(r'\w', name):
			...
			continue
		target = ' '.join([drugname, ingredient])
		...
		for ing in inglist:
			if not re.search(ing[0], target): continue
			unii = uniis[ing[0]]
			...
			for xml in unii2xml[unii]:
				if not xml in supportInDailyMed[curie]: continue
				xml = xml.lstrip('xmls/')
			...
			saveEdge(savedEdges, efile, 
				...
				ing[0], name,
				'(qual)', knowledge_level, agent_type,
				approval_code, n, SPLlist
				)
			...
			if SPLlist:
				...
				#agent_type = 'text_mining_agent'
				approval_code = display_nda
				saveEdge(savedEdges, efile, 
					unii, predicate, curie,
					ing[0], name,
					'(qual)', knowledge_level, agent_type,
					approval_code, 0, SPLlist
					)
