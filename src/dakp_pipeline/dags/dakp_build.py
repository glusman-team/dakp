...
def dakp_build() -> None:
    ...
    def extract_dailymed(raw: list[dict[str, Any]]) -> list[dict[str, Any]]: ...

    @task.stub(queue=GO_QUEUE, pool=EXTRACT_POOL)
    def extract_faers(raw: list[dict[str, Any]]) -> list[dict[str, Any]]: ...

    @task.stub(queue=GO_QUEUE, pool=EXTRACT_POOL)
    def extract_drugsfda(raw: list[dict[str, Any]]) -> list[dict[str, Any]]: ...

    # -- assertion shaping (Python) ---------------------------------------------
    @task
    def shape_treatment_tables(dm_ext: list[dict[str, Any]], drugsfda_ext: list[dict[str, Any]]) -> list[dict[str, Any]]:
        refs = [*refs_from_xcom(dm_ext), *refs_from_xcom(drugsfda_ext)]
        return refs_to_xcom(approved_treats.transform(refs, _ctx()))

    @task
    def shape_faers_use_tables(faers_ext: list[dict[str, Any]], dm_ext: list[dict[str, Any]]) -> list[dict[str, Any]]:
        refs = [*refs_from_xcom(faers_ext), *refs_from_xcom(dm_ext)]
        return refs_to_xcom(observed_uses.transform(refs, _ctx()))

    @task
    def shape_contraindication_tables(dm_ext: list[dict[str, Any]], ner_models: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def generate_tablassert_configs(
        approved: list[dict[str, Any]], uses: list[dict[str, Any]], contra: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def write_build_summary(
        approved: list[dict[str, Any]], uses: list[dict[str, Any]], contra: list[dict[str, Any]], kgx: list[dict[str, Any]]
    ) -> str:
        cfg = _cfg()
        assertion_refs = [*refs_from_xcom(approved), *refs_from_xcom(uses), *refs_from_xcom(contra)]
        report = translator_contract.validate(assertion_refs)
        regression_report = regression.check_assertion_tables(assertion_refs)
        summary = runtime.write_build_summary(cfg, assertion_refs, refs_from_xcom(kgx), report, regression_report)
        return str(summary)
