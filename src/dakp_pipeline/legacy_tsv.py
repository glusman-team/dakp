...
def _endpoint_name(edge: Mapping[str, Any], side: str, names: Mapping[str, str]) -> str:
    ...
    for an endpoint that somehow missed the node set. The fallback preserves the raw assertion
    mention when canonical node data is unavailable; ``--qc`` normally prevents undeclared nodes.
    """
