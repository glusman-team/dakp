...
def test_edges_carry_dakp_provenance(kgx_build: KgxBuild) -> None:
    """RAW Tablassert edges carry DAKP as primary for every edge family.
...
def test_edge_evidence_lands_on_the_edge_not_in_a_study(kgx_build: KgxBuild) -> None:
    ...
    for edge in kgx_build.edges:
        for field in ("regulatory_approvals", "publications"):
            if field in edge:
...
def test_regulatory_approvals_ride_the_edge_as_a_top_level_json_array(kgx_build: KgxBuild) -> None:
    ...
    assert any("regulatory_approvals" in edge for edge in edges), "no edge carries regulatory_approvals as its own field"
    for edge in edges:
        approvals = edge.get("regulatory_approvals")
        if approvals is None:
            # Missing source approval provenance is intentionally omitted, not represented by [].
            continue
        assert isinstance(approvals, list), f"regulatory_approvals missing or not a JSON array: {approvals!r}"
        assert approvals, "populated regulatory_approvals must not be an empty array"
        for value in approvals:
            assert isinstance(value, str)
            assert value.strip(), f"empty approval id in {approvals!r}"
            assert "|" not in value, f"regulatory_approvals kept a joined cell instead of splitting it: {value!r}"
        assert "regulatory_approvals" not in (edge.get("supporting_text") or "")
...
def test_legacy_tsv_pair_matches_the_old_schema(kgx_build: KgxBuild) -> None:
    ...
    node categories are first-element; edge multi-values are comma-joined (`regulatory_approvals` ->
    ...
    for line in edges_lines[1:]:
        ...
        assert row["approval"] == (",".join(edge["regulatory_approvals"]) if "regulatory_approvals" in edge else "NA")
