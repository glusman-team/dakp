...
def test_up_config_variable_carries_null_limits_and_fullmap(monkeypatch: pytest.MonkeyPatch, sandbox: Path) -> None:
    ...
    assert "no_original" not in config
...
def test_up_small_sets_scope_bounds(monkeypatch: pytest.MonkeyPatch, sandbox: Path) -> None:
    ...
    assert config["qc"] is True  # QC audit is scope-independent too
...
def offline_ner(monkeypatch: pytest.MonkeyPatch):
    ...
    ``run_export_ner`` wires the real GLiNER2 backend; unit tests must not load torch, so the
    ``for_contraindications`` constructor is patched to the offline gazetteer backend (a real
    ``DiseaseNER``, so the export's ``isinstance`` injection check passes).
    """
    from dakp_pipeline.assertions.ner_dispatch import default_ner
...
def _write_interim_tables(workdir: Path) -> None:
    ...
    (interim / "ema").mkdir(parents=True, exist_ok=True)
    ...
    pl.DataFrame(
        {
            ...
            "active_ingredient_name": ["LIVERDRUG", ""],
        }
    ).write_parquet(interim / "dailymed" / "spl_documents.parquet")
    pl.DataFrame(
        {
            "therapeutic_indication": ["Treatment of breast cancer."],
            "active_substance": ["SOMESUBSTANCE"],
            "inn": [""],
            "ema_product_number": ["EMEA-000001"],
            "medicine_url": ["https://www.ema.europa.eu/medicines/epar-1"],
        }
    ).write_parquet(interim / "ema" / "ema_registry.parquet")
    pl.DataFrame(
        {
            "quarter": ["24Q3", "24Q3"],
            "primaryid": ["1001", "1002"],
...
def test_export_ner_happy_path_copies_to_out(sandbox: Path, capsys: pytest.CaptureFixture[str], offline_ner) -> None:
    ...
    code = cli.run_export_ner(out=str(out), workdir=str(workdir))

    assert code == 0
    assert "bundle ready" in capsys.readouterr().out
    manifest = json.loads((out / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["schema_version"] == "dakp.ner.export.v1"
    # 2 DailyMed sections (one per export LOINC) + 1 EMA indication + 2 FAERS indications.
    assert manifest["files"]["examples.ndjson"]["rows"] == 5
    assert manifest["files"]["examples.avro"]["examples"] == 5
    assert manifest["task_counts"] == {"contraindication": 1, "indication": 4}
    assert manifest["family_counts"] == {"dailymed": 2, "ema": 1, "faers": 2}
...
def test_export_ner_default_out_is_the_store_bundle(sandbox: Path, offline_ner) -> None:
    ...
    assert code == 0
    bundle = sandbox / "work" / "store" / "ner-export"
    assert sorted(path.name for path in bundle.iterdir()) == ["examples.avro", "examples.ndjson", "manifest.json", "ner_gold.json"]
    # The NDJSON projection is newline-delimited JSON objects, one per line.
    lines = (bundle / "examples.ndjson").read_text(encoding="utf-8").splitlines()
    assert len(lines) == 5
