...
def test_one_call_emits_both_channels_with_distinct_provenance(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert [(m.text, m.type, m.notes) for m in mentions] == [
        ...
        ("women", TYPE_BIOLOGICAL_SEX, QUALIFIER_NOTES),
    ]
...
def test_population_qualifier_survives_the_object_population_filter(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert [(m.text, m.type, m.notes) for m in qualifier] == [(phrase, TYPE_POPULATION_OF_INDIVIDUAL_ORGANISMS, QUALIFIER_NOTES)]

    _install_fake_gliner2(monkeypatch, tmp_path, [_span(text, phrase, "biolink:PhenotypicFeature", 0.86)])
    assert _backend().extract(text) == []  # object channel: population descriptor, dropped as before
...
def test_offline_mode_emits_object_channel_only() -> None:
    """The deterministic offline backend has no model and the embedded gazetteer holds no qualifier
    terms, so it can never emit a qualifier mention — offline output stays a pure object channel."""
    mentions = DiseaseNER().extract("Contraindicated in women of childbearing potential with asthma, once daily.")
    assert [m.text for m in mentions] == ["asthma"]
    assert {m.type for m in mentions} <= set(OBJECT_TYPES)
    assert all(m.notes != QUALIFIER_NOTES for m in mentions)
