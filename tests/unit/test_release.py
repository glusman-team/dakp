...
``drug_approvals_kg_v<version>.yaml``), the deferred-handoff empty return, and the loud
``RuntimeError`` guards on a missing/ambiguous handoff report, legacy TSV pair, or graph config.
"""
...
def _scaffold(workdir: Path, *, mode: str = "real") -> tuple[list[ArtifactRef], list[ArtifactRef], list[ArtifactRef]]:
    """Write the handoff report + KGX pair + legacy TSV pair + graph config; return the ref lists."""
    data = workdir / "data"
    ...
    graph = workdir / "tables" / "graph.yaml"
    graph.parent.mkdir(parents=True, exist_ok=True)
    graph.write_text("name: DRUG_APPROVALS_KP\n", encoding="utf-8")
    return kgx_refs, legacy_refs, [_ref(graph, "application/yaml")]
...
def test_legacy_name_schema() -> None:
    ...
    data_names = {
        ...
        f"drug_approvals_kg_v{__version__}.yaml",
    }
...
def test_publish_copies_everything_under_the_legacy_names(tmp_path: Path) -> None:
    kgx_refs, legacy_refs, config_refs = _scaffold(tmp_path)
    refs = publish(kgx_refs, legacy_refs, config_refs, _ctx(tmp_path))
    ...
    assert (data / expected[-1]).read_text(encoding="utf-8") == "name: DRUG_APPROVALS_KP\n"
    assert all(ref.blake3.startswith("b3:") for ref in refs)


def test_publish_deferred_handoff_returns_empty(tmp_path: Path) -> None:
    kgx_refs, legacy_refs, config_refs = _scaffold(tmp_path, mode="deferred")
    assert publish(kgx_refs, legacy_refs, config_refs, _ctx(tmp_path)) == []


def test_publish_requires_exactly_one_report(tmp_path: Path) -> None:
    _kgx_refs, legacy_refs, config_refs = _scaffold(tmp_path)
    with pytest.raises(RuntimeError, match="exactly one"):
        publish([], legacy_refs, config_refs, _ctx(tmp_path))
    kgx_refs, _, _ = _scaffold(tmp_path)
    with pytest.raises(RuntimeError, match="exactly one"):
        publish([*kgx_refs, kgx_refs[0]], legacy_refs, config_refs, _ctx(tmp_path))
...
def test_publish_requires_the_legacy_tsv_pair(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match=r"legacy \.nodes\.tsv/\.edges\.tsv pair"):
        publish(kgx_refs, legacy_refs[:1], config_refs, _ctx(tmp_path))


def test_publish_requires_the_graph_yaml(tmp_path: Path) -> None:
    kgx_refs, legacy_refs, _config_refs = _scaffold(tmp_path)
    with pytest.raises(RuntimeError, match=r"exactly one graph\.yaml"):
...
def test_publish_requires_the_kgx_pair_on_disk(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="exactly one"):
        publish(kgx_refs, legacy_refs, config_refs, _ctx(tmp_path))
